...
def test_decompose_preserves_project_scope_with_unique_worktrees(kanban_home, tmp_path):
    ...
    with pdb.connect_closing() as pconn:
        project_id = pdb.create_project(pconn, name="Widget Project", primary_path=str(repo))
    kb.create_board("scoped", name="Scoped", project_id=project_id)
    with kb.connect(board="scoped") as conn:
        root_id = kb.create_task(
            conn, title="Refactor widget project", triage=True, board="scoped",
        )
        child_ids = kb.decompose_triage_task(
            conn, root_id, root_assignee="orchestrator",
            children=[
