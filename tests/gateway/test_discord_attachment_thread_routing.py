...
class TestAttachmentDestinationRouting:
    @pytest.mark.parametrize(
        "failure", [ImportError("missing aiohttp"), RuntimeError("download failed")]
    )
    async def test_remote_image_fallback_preserves_thread_metadata(
        self, adapter, monkeypatch, failure
    ):
        instance, _, _ = adapter
        ...
        fallback = AsyncMock(
            return_value=SimpleNamespace(success=True, message_id="fallback")
        )
        monkeypatch.setattr(BasePlatformAdapter, "send_image", fallback)

        metadata = {"thread_id": "200"}
        result = await instance.send_image(
            "100", "https://cdn.example.test/image.png", metadata=metadata
        )
