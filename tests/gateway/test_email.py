...
class TestEmailResponseDelivery(unittest.TestCase):
    ...
    def test_profile_scoped_email_redirect_suppresses_home_notice(self):
        from agent.secret_scope import (
            reset_secret_scope,
            set_multiplex_active,
            set_secret_scope,
        ...
        with patch.dict(os.environ, {
            ...
            set_multiplex_active(True)
            token = set_secret_scope({"EMAIL_RESPONSE_DELIVERY": "discord"})
            try:
                self.assertTrue(_suppress_home_channel_notice("email"))
            finally:
                reset_secret_scope(token)
    ...
    def test_profile_without_redirect_does_not_inherit_global_suppression(self):
        from agent.secret_scope import (
            reset_secret_scope,
            set_multiplex_active,
            set_secret_scope,
        ...
        with patch.dict(os.environ, {
            ...
            try:
                self.assertFalse(_suppress_home_channel_notice("email"))
            finally:
                reset_secret_scope(token)
                set_multiplex_active(False)
